...
const searchParams = useSearchParams()
  const [seniorFilter, setSeniorFilter] = useState(searchParams.get("seniorId") ?? "all")
  ...
  // "routine" = care memberswho have zero critical/monitor active alerts
  const routineCount = MOCK_SENIORS.filter((s) =>
    !active.some(
  ...
  const filtered = alerts
    .filter((a) => {
      ...
      if (!isResolved && !showActive) return false
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {active.length} Active Alerts Across {MOCK_SENIORS.length} Care Members
          </p>
        </div>
        {selectedIds.size > 0 && (
          <Button
...
{/* ── Active / Resolved toggles ── */}
      <div className="flex gap-2 mb-4">
        <button
          onClick={() => setShowActive(!showActive)}
          className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${showActive ? 'bg-[#3B5BDB] text-white border-[#3B5BDB]' : 'bg-white text-gray-600 border-gray-200 dark:bg-gray-900 dark:text-gray-400 dark:border-gray-700'}`}
        >
          Active Alerts
        </button>
        <button
          onClick={() => setShowResolved(!showResolved)}
          className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${showResolved ? 'bg-[#3B5BDB] text-white border-[#3B5BDB]' : 'bg-white text-gray-600 border-gray-200 dark:bg-gray-900 dark:text-gray-400 dark:border-gray-700'}`}
        >
          Resolved Alerts
        </button>
      </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {alert.severity === "routine" ? (
                          <span className="inline-flex items-center text-xs font-medium px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400">{severityLabel[alert.severity]}</span>
                        ) : (
...
/**
 * Wraps `AlertsContent` in a Suspense boundary because it reads
 * `useSearchParams()`. Next 14's static prerender bails out unless
 * the consumer is inside a Suspense — keeping the boundary at the
 * page root avoids leaking the search-params dependency into nested
 * layouts.
 */
