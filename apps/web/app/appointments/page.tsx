...
function DaysUntilBadge({ days }: { days: number }) {
  ...
  if (days <= 2) return <span className="text-xs font-medium px-2 py-1 rounded-full bg-red-100 text-red-700">{days} days</span>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">View All Upcoming Appointments Across Your Care Circle</p>
      </div>
          ...
          {filtered.map(appt => {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">{senior.primaryConditions.join(", ")}</p>
                    </div>
                  </div>
                ...
                <p className="text-sm text-gray-500 mb-1">{formatDate(appt.dateTime)}</p>
                {appt.location && (
...
/**
 * Wraps `AppointmentsContent` in a Suspense boundary because it reads
 * `useSearchParams()`. Required by Next 14's static prerender — see
 * the matching wrapper in `app/alerts/page.tsx` for the same rationale.
 */
